 ...
 * Note: The Volume Profile is currently not displaying correctly and will need to be fixed in a future update.
 */
...
const HeikinAshiVolumeProfile: React.FC<HeikinAshiVolumeProfileProps> = ({ historicalData }) => {
  ...
  useEffect(() => {
    ...
    if (historicalData.length > 0 && chartContainerRef.current) {
      ...
      // Note: Volume Profile display is not working correctly and needs to be fixed
      // For now, we'll just log the calculated data
      console.log('Volume Profile Data:', vpData);
  ...
  return (
    <div className="bg-white shadow-md rounded-lg p-6">
      ...
      <div className="mt-4 text-sm text-gray-600">
        ...
        <p className="mt-3"><span className="font-semibold">Note:</span> The Volume Profile overlay is currently not displaying correctly and will be fixed in a future update. The chart still provides valuable insights through the Heikin-Ashi candlesticks.</p>
      </div>
    </div>
  );
};
